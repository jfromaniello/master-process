...
module.exports = function monit (worker, debug, fork) {
  ...
  worker.on('message', function (message) {
    ...
    if (command.msg === 'pause_monitoring') {
      paused = true;
      console.log('%s pause monitoring', new Date().toISOString());
    }
    if (command.msg === 'resume_monitoring') {
      paused = false;
      console.log('%s resume monitoring', new Date().toISOString());
    }
  });
  ...
  setTimeout(function(){
    ...
    monitor = setInterval(function () {
      ...
      usage.lookup(proc.pid, {
        ...
        if (err) {
          console.error('%s %s', new Date().toISOString(), util.inspect(err));
