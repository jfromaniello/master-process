...
module.exports = function monit (worker, debug, fork) {
  ...
  const max_mem_failures = parseInt(process.env.MEM_MONITOR_FAILURES, 10) || 10;
  const max_cpu_failures = parseInt(process.env.CPU_MONITOR_FAILURES, 10) || 10;
  let max_memory = DEFAULT_MAX_MEMORY;
  const max_kill_timeout = parseInt(process.env.MAX_KILL_TIMEOUT, 10) || 5000;
  ...
  function kill_child_and_restart(resource) {
  ...
  setTimeout(function(){
    ...
    monitor = setInterval(function () {
      ...
      usage.lookup(proc.pid, {
        ...
        const mem_perc = result.memory * 100 / max_memory;
